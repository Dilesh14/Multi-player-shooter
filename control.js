...
var entity = function () {
  ...
  self.getdist = function (pt) {
    return Math.sqrt(
      Math.pow(self.posx - pt.posx, 2) + Math.pow(self.posy - pt.posy, 2)
    );
  };
  self.getdistbul = function (pt) {
    return Math.sqrt(
      Math.pow(self.posx - 5 - pt.posx, 2) +
        Math.pow(self.posy - 5 - pt.posy, 2)
    );
  };
  self.getdistrect = function (pt) {
    return Math.sqrt(
      Math.pow(self.posx - 5 - pt.posx - pt.width / 2, 2) +
        Math.pow(self.posy - 5 - pt.posy - pt.height / 2, 2)
    );
  };
  return self;
};
var Player = function (id) {
  ...
  self.updateSpeed = function () {
    if (self.goDown) self.speedy = self.maxspeed;
    else if (self.goUp) self.speedy = -1 * self.maxspeed;
    else self.speedy = 0;
    if (self.goLeft) self.speedx = -1 * self.maxspeed;
    else if (self.goRight) self.speedx = self.maxspeed;
    else self.speedx = 0;
  };
  self.checkpos = function () {
    if (self.posx < self.radius) self.posx = 0 + self.radius;
    if (self.posx > 700 - self.radius) self.posx = 700 - self.radius;
    if (self.posy < self.radius) self.posy = 0 + self.radius;
    if (self.posy > 600 - self.radius) self.posy = 600 - self.radius;
  };
  Player.list[id] = self;
  return self;
};
...
Player.connect = function (socket) {
  ...
  socket.on("keyPressed", function (state) {
    if (state.input == "up") player.goUp = state.press;
    if (state.input == "right") player.goRight = state.press;
    if (state.input == "left") player.goLeft = state.press;
    if (state.input == "down") player.goDown = state.press;
    if (state.input == "attack") player.attack = state.press;
...
Player.update = function () {
  ...
  for (var i in Player.list) {
    ...
    if (player.respawn) {
      ...
      player.score = 0;
      player.radius = 11;
      player.posx = Math.floor(
        Math.random() * (695 - player.radius) + player.radius
      );
      player.posy = Math.floor(
        Math.random() * (595 - player.radius) + player.radius
      );
    }
    playerPack.push({
      x: player.posx,
...
var Bullet = function (parent, tangle) {
  ...
  var entity_update = self.update;
  self.update = function () {
    if (self.time++ > 20) self.dead = true;
    entity_update();
    for (var i in Player.list) {
      var p = Player.list[i];
...
let Energy = function () {
  ...
  self.update = function () {
    entity_update();
    if (self.timer++ > 100) self.dead = true;
    for (var i in Player.list) {
      var p = Player.list[i];
...
setInterval(function () {
  if (Math.random() < 0.04) {
    if (Object.keys(Enemy.list).length < 100) Enemy();
  }
  if (Math.random() < 0.01) {
    Energy();
  }
    ...
    for (let i in Player.list) {
      if (i == socket.id) {
        socket.emit("score", {
          ...
          highscore: Player.list[i].highscore,
        });
      }
    }
  }
